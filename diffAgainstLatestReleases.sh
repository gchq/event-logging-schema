...
if [ "$#" -ne 1 ] || ! [ -d "$1" ]; then
    ...
    echo -e "Where workingDir is normally ${BLUE}event-logging-transformer-main/pipelines/generated${NC}"
    exit 1
fi
...
while read -r url; do
    ...
    echo -e "Downloaded file: [${GREEN}${downloadedFilename}${NC}]"
    echo -e "Suffix: [${GREEN}${suffix}${NC}]"

    #localFile=$(ls ../event-logging-v*${suffix}.xsd)
    localFile="$(find ../ -maxdepth 1 -regex ".*/event-logging-v[0-9]*${suffix}.xsd")"
    if [ "x${localFile}" = "x" ]; then 
        echo -e "${RED}Warning${NC}: no local file found to match downloaded file ${BLUE}${downloadedFilename}${NC}"
    else
        echo -e "Local file: [${GREEN}${localFile}${NC}]"

        echo -e "Diffing ${BLUE}${localFile}${NC} against ${BLUE}${downloadedFilename}${NC}"
        diffFile="${downloadedFilename}.diff"
        #OR with true to stop the exit code from diff stopping the script
        diff ${localFile} ${downloadedFilename} > ${diffFile} || true
        if [ $(cat ${diffFile} | wc -l) -gt 0 ]; then
            diff ${localFile} ${downloadedFilename} || true
            echo -e "\n${RED}Warning${NC}: Local schema ${BLUE}${localFile}${NC} differs from ${BLUE}${downloadedFilename}${NC}"
            echo -e "(see changes in $PWD/${diffFile})${NC}"
        else 
            echo -e "\n${GREEN}Local schema ${BLUE}${localFile}${NC} is identical to ${BLUE}${downloadedFilename}${NC}"
        fi
    fi
done <<< "$latestUrls"
