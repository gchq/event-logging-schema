...
An IPS appliance blocks an IP address that may be attempting an SQL injection attack.|```<Alert>``` with sub-element ```<IDS>```.  Attacker IP is recorded in ```<Source>``` and target in ```<Destination>```.|
A database reports that a processing account logs on and performs a backup.|```<Authenticate>``` and then a second event, ideally ```<Process>``` if the backup is contained within the database and control maintained, otherwise ```<Export>```.| It might be unclear that this is a backup operation, e.g. perhaps a query is performed.  In this case ```<Search>``` may be most appropriate for the second event.
