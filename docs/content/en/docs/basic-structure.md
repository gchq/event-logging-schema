...
All events should be marked with the version of the schema that they are based upon using the `Version` attribute of the `<Events>` element. When an `<Event>` is transmitted as a fragment without its enclosing `<Events>` element, the optional `Version` attribute on `<Event>` can be used instead.
